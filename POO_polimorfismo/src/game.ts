...
export function game(): void {
  ...
  function combate(player: Heroe, cpu: Heroe): void {
    console.log(`${player.nombre} se enfrentara a ${cpu.nombre}... `);
    ...
    while (player.vida > 0 && cpu.vida > 0) {
      ...
      while (isNaN(accionJugador) || accionJugador < 1 || accionJugador > 5) {
        console.log("Error: Debes ingresar un numero valido entre 1 y 5.");
        accionJugador = Number(readlineSync.question("Por favor, elige nuevamente: "));
      }

      switch (accionJugador) {
        case 1:
          console.log("El jugador realiza un ataque fisico");
          player.ataqueFisico(cpu);
          break;
        case 2:
          console.log("El jugador realiza un ataque magico");
          player.ataqueMagico(cpu);
          break;
        case 3:
          console.log("El jugador aumenta su defensa fisica");
          player.defensaFisica();
          break;
        case 4:
          console.log("El jugador aumenta su defensa magica");
          player.defensaMagica();
          break;
        case 5:
          console.log("El jugador se cura");
          player.curar();
          break;
        default:
          throw new Error("Error inesperado al seleccionar accion del jugador.");
      }

      // Verificar si la CPU ha sido derrotada
      if (cpu.vida <= 0) {
        console.log(`${cpu.nombre} ha sido derrotado. ¡Has ganado esta ronda!`);
      ...
      switch (accionCPU) {
        case 1:
          console.log("La CPU realiza un ataque fisico");
          cpu.ataqueFisico(player);
          break;
        case 2:
          console.log("La CPU realiza un ataque magico");
          cpu.ataqueMagico(player);
          break;
        case 3:
          console.log("La CPU aumenta su defensa fisica");
          cpu.defensaFisica();
          break;
        case 4:
          console.log("La CPU aumenta su defensa magica");
          cpu.defensaMagica();
          break;
        case 5:
          console.log("La CPU se cura");
          cpu.curar();
          break;
        default:
          throw new Error("Error inesperado al seleccionar accion del CPU.");
      }

      // Verificar si el jugador ha sido derrotado
      if (player.vida <= 0) {
        console.log(`${player.nombre} ha sido derrotado. ¿Deseas jugar de nuevo? (S/N)`);
        let reiniciar = readlineSync.question("Ingresa S para jugar de nuevo o N para salir: ").toUpperCase();
        if (reiniciar === 'S') {
          game();
          } else {
            process.exit();
      ...
      console.log(`Vida de ${player.nombre}: ${player.vida}`);
      console.log(`Vida de ${cpu.nombre}: ${cpu.vida}`);
    }
  }
